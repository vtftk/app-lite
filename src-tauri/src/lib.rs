...
use state::{
    app_data::{
        ...
        MinimumRequireRole, ThrowableConfig, ThrowableImageConfig,
    },
    ...
    while let Ok(event) = twitch_event_rx.recv().await {
        ...
        match event {
            TwitchEvent::Redeem(event) => {
                handle_redeem_event(app_data, twitch_manager.clone(), &event_sender, event)
            }
            TwitchEvent::CheerBits(event) => {
                handle_cheer_bits_event(app_data, twitch_manager.clone(), &event_sender, event)
            }
            TwitchEvent::Follow(event) => {
                handle_follow_event(app_data, twitch_manager.clone(), &event_sender, event)
            }
            TwitchEvent::Sub(event) => {
                handle_sub_event(app_data, twitch_manager.clone(), &event_sender, event)
            }
            TwitchEvent::GiftSub(event) => {
                handle_gift_sub_event(app_data, twitch_manager.clone(), &event_sender, event)
            }
            TwitchEvent::ResubMsg(event) => {
                handle_resub_event(app_data, twitch_manager.clone(), &event_sender, event)
            }
